...
NOTE: a logically-identical copy lives in ``telemetry/anon_name.py`` on the
client -- the SAME word lists + algorithm (the server package is import-isolated
from ``telemetry/``, so the code is duplicated rather than shared). The two are
NOT byte-identical (the docstrings differ); the SERVER computes the canonical
public board name with ``lang='en'``. A shared EN/DE test vector pins the OUTPUT
